...
- The full test suite (119 tests) runs without any network access, using a canned-response stub LLM client.
...
- SQLite lineage: `evaluation_runs`, `metric_results`, `case_results` tables with run → trial linkage; `dspy_compiles` for compile lineage (separate from evaluation runs).
- CLI (`crucible evaluate <app>`, `crucible optimize <app> [--trials N]`, `crucible compile <app> [--max-examples N]`, `crucible evaluate <app> --program <path>`) and Makefile targets.
