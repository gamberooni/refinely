...
- Optuna-based optimization: per-app search spaces, TPE sampler, SQLite storage shared with the lineage database, resumable studies (`refinely_{app}`, `load_if_exists=True`).
...
- CLI, all rendered with `rich` tables/panels, plus Makefile targets:
  - `refinely evaluate <app> [--config <name|json>] [--model <name>] [--models a,b,c] [--tags a,b] [--program <path>]` — run a single evaluation, or fan out across models (`--models`).
  - `refinely optimize <app> [--trials N] [--model <name>] [--tags a,b]` — Optuna search; the best trial's config is auto-saved to `configs/<app>/opt-best.json`.
  - `refinely compile <app> [--max-examples N]` — optional DSPy behavior optimization.
  - `refinely show <app> [--run <run_id>] [--page N] [--pager] [--tag ...]` — run history and per-case results (with error column + "N cases errored").
  - `refinely compare <app> [--baseline <run_id>] [--model ...] [--tag ...] [--diff-config] [--cases] [--pager]` — per-metric deltas, config deltas, and per-case regression drill-down.
  - `refinely export <app> [--format csv|json] [--output FILE] [--tag ...]`.
  - `refinely config` (`save`/`list`/`show`/`rm`/`default`) — manage named configs; `refinely new app <name>` — scaffold; `refinely doctor [--network]` — health checks; `refinely dataset stats <app>` — dataset statistics.
...
- **Optional DSPy integration** (install with `uv sync --group dspy`): each app can declare a `dspy_factory` on its `AppRegistration` returning a `DspyProgramSpec` (three callables: `build`, `prepare_example`, `prediction_to_output`); `refinely compile` runs `BootstrapFewShot` against the app's dataset and registered metrics; compile artifacts are JSON files loadable at evaluate time via `--program`.
- Configuration via environment / `.env` (`REFINELY_*` prefix, `OPENAI_API_KEY` fallback, optional OpenAI-compatible `base_url` for local gateways).
...
- **Serving**: no HTTP server, no API surface for running apps; Refinely is a CLI/experimentation tool, not a deployment platform.
...
- **Observability tooling**: no dashboards, charts, or web UI — lineage read-back is provided by the `refinely show/compare/export` commands (terminal tables and CSV/JSON file export), and raw `sqlite3` queries (an example query is in the README) remain available.
