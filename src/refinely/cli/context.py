...
``refinely.cli.context.get_registration``) and every command picks up the
...
from refinely.eval.datasets import EvalCase, dataset_version, load_dataset
from refinely.llm.client import AsyncOpenAIClient, LLMClient
from refinely.optimize.study import run_study
from refinely.registry import AppRegistration, get_registration, registered_apps
...
def _client(settings: Settings) -> LLMClient:
    if not settings.has_api_key:
        raise click.ClickException(
            "REFINELY_OPENAI_API_KEY is not set; set the env var or add a .env file"
        )
