"""Health checks for the crucible development environment.
...
from crucible.registry import discover_apps, registered_apps
...
def _check_apps() -> CheckResult:
    ...
    if not apps:
        return CheckResult(
            ...
            "declare an entry point under [project.entry-points.\"crucible.apps\"] "
            "in pyproject.toml, or run 'crucible new app <name>'",
        )
...
def _check_env(settings: Settings) -> CheckResult:
    ...
    return CheckResult(
        "env", False, "CRUCIBLE_OPENAI_API_KEY is not set",
        "set CRUCIBLE_OPENAI_API_KEY in the environment or add a .env file",
    )
