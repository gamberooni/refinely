...
- Add `crucible show <app>` — rich table of recent runs (newest first) plus best run and best compile summary.
- Add `crucible show <app> --run <run_id>` — per-case drill-down, worst cases first (uses existing `case_results_for_run`).
- Add `crucible compare <app>` — metric table comparing each run against the previous run (chronological), with optional `--baseline <run_id>` to compare against a specific run instead.
- Add `crucible export <app> [--format csv|json] [--output FILE]` — writes runs + metrics to a file; `--format` defaults to `csv`; `--output` defaults to `<app>_runs.csv` (or `.json`) in the current directory; always writes a file.
...
- **Code**: `src/crucible/cli.py` (3 new commands + output formatting), `src/crucible/tracking/db.py` (one new query method), `src/crucible/reporting/` (new module for rendering/export logic).
