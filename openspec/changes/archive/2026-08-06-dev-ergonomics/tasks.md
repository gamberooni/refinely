...
- [x] 1.1 Add `src/refinely/scaffold.py` with app-module and dataset-stub templates following the `apps/extraction.py` convention (DATASET_PATH via `parents[1] / "datasets" / "<name>_v1.json"`, register_app skeleton with TODO placeholders for build_adapter, metrics_factory, search_space, default_config, weights, dataset_path) (D1)
...
- [x] 1.3 Add `new app` click subcommand to `cli.py`: `refinely new app <name> [--dataset <path>]`; with `--dataset`, point dataset_path at it and skip the stub; always print the `[project.entry-points."refinely.apps"]` line to add; never edit pyproject.toml (D5)

## 2. Doctor

- [x] 2.1 Add `src/refinely/doctor.py` with a `CheckResult` model and check functions: apps (discover_apps ≥1), datasets (load_dataset per registered app), schema (LineageDB open → init_schema), env (settings.openai_api_key non-empty) (D3)
...
- [x] 3.1 Add `dataset_stats(path) -> DatasetStats` to `src/refinely/eval/datasets.py`: case count, file_size_bytes, input-key presence counts, expected type/key histogram, malformed-case report (modal-shape deviation), never raising on structural inconsistencies; parse errors propagate (D4)
