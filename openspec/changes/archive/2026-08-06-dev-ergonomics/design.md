...
Contributing an app to Crucible today means hand-writing `apps/<name>.py` with a `register_app(AppRegistration(...))` call (six required fields), creating a dataset by hand, and adding an entry point under `[project.entry-points."crucible.apps"]` in `pyproject.toml`. There is no scaffold, no health check, and no way to inspect a dataset before running an evaluation. `load_dataset` (src/crucible/eval/datasets.py) already validates structure at load time with case-indexed `EvalError`s — so parse-level validation exists; what is missing is *inspection* (stats) and *guidance* (scaffold/doctor).

Apps live outside the crucible package (`apps/`, sibling of `src/`), are loaded via `discover_apps()` over entry points, and register via `register_app`. All existing CLI commands funnel through `cli.py`'s shared `_load_run_context(app)` helper.
...
### D1: New `src/crucible/scaffold.py` with string templates
...
### D3: New `src/crucible/doctor.py` — check list + exit codes
...
4. **env** — `settings.openai_api_key` present (non-empty), using the existing `CRUCIBLE_OPENAI_API_KEY is not set` semantics.
