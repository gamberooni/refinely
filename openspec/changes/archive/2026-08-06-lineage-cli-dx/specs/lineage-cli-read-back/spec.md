...
The system SHALL support `refinely show <app> --run <run_id>` to render the run's per-case results as a table, ordered by case score ascending (worst cases first), with case id, score, input, expected, and output.
...
- **WHEN** a user runs `refinely export <app> --format yaml`
