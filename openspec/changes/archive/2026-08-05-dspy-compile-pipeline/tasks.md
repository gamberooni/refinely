...
- [x] 2.1 Create `src/crucible/dspy/__init__.py` + `spec.py` with `DspyProgramSpec` dataclass (`build`, `prepare_example`, `prediction_to_output`)
...
- [x] 3.1 `src/crucible/dspy/bridge.py`: metric bridge (prediction → synthetic Result → app's metrics via EvaluationRunner scoring → weighted aggregate)
- [x] 3.2 `src/crucible/dspy/lm.py`: `configure_lm(settings, temperature)` via `dspy.LM` with OpenAI-compatible `api_base`
- [x] 3.3 `src/crucible/dspy/compile.py`: train/val split (70/30, `--max-examples` cap), BootstrapFewShot runner, baseline + compiled eval on val, artifact save (`optimized_program.json`), returns result dataclass
...
- [x] 6.1 `crucible compile <app>` subcommand (flags: `--max-examples`, `--max-rounds`, `--max-labeled-demos`, `--max-bootstrapped-demos`, `--output-dir`, `--lineage-db`; clear errors for no-dspy_factory and dspy-not-installed)
- [x] 6.2 `crucible evaluate --program <path>` flag forwarded to `build_adapter`
...
- [x] 7.4 Full suite green (~100 tests) + real-API smoke `crucible compile extraction` against local gateway
