...
- Add a `click`-based CLI (`crucible`) to run evaluations and optimizations per app.
...
- `cli`: The `crucible` command-line entrypoint for running evaluations and optimizations against either toy application.
...
- **New code**: `src/crucible/core/` (settings, exceptions), `src/crucible/llm/` (client, usage), `src/crucible/apps/` (protocol, extraction, qa), `src/crucible/datasets/` (loader + JSON fixtures), `src/crucible/eval/` (runner, metrics), `src/crucible/optimize/` (search space, objective), `src/crucible/tracking/` (SQLite db), `src/crucible/cli.py`.
...
- **No production impact**: this is a standalone prototype in `src/crucible/`; it does not touch or depend on the enterprise platform described in `docs/`.
