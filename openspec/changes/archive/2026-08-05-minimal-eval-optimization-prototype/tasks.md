...
- [x] 1.2 Add `[project.scripts]` entry `refinely = "refinely.cli:main"` to `pyproject.toml`
- [x] 1.3 Create package skeleton: `src/refinely/{core,llm,apps,datasets,eval,optimize,tracking}/__init__.py`
- [x] 1.4 Install dependencies and verify `python -c "import refinely"` succeeds

## 2. Core: Settings and Exceptions

- [x] 2.1 Implement `refinely/core/exceptions.py` with `LLMError` and `EvalError` exception classes
- [x] 2.2 Implement `refinely/core/settings.py` using `pydantic-settings` `BaseSettings` with fields for OpenAI API key, model name, and lineage DB path, loading from environment / `.env`
- [x] 2.3 Unit test: settings load correctly from environment variables (using monkeypatched env, no real API key required)

## 3. LLM Client

- [x] 3.1 Implement `refinely/llm/usage.py` with the `TokenUsage` model (`prompt_tokens`, `completion_tokens`)
- [x] 3.2 Implement `refinely/llm/client.py`: adapt the `AsyncOpenAIClient` pattern (rename imports to `refinely`), including `LLMClient` Protocol, `chat_structured` (JSON-schema-forced output, fence-stripping, repair-retry), `chat_text`, `tenacity` retry wrapping, and per-model temperature support detection
...
- [x] 4.1 Implement `refinely/apps/protocol.py`: `ApplicationAdapter` Protocol with `execute(input: dict, config: dict) -> Result`, and a shared `Result` model (output + execution metadata including token usage and latency)

## 5. Extraction Application

- [x] 5.1 Implement `refinely/apps/extraction.py`: `ExtractionApp` implementing `ApplicationAdapter`, using `chat_structured` with a Pydantic extraction response model, `temperature` and `system_prompt_variant` config knobs (two hand-written prompt templates: "strict"/"verbose")
...
- [x] 6.2 Implement `refinely/apps/qa.py`: `QAApp` implementing `ApplicationAdapter`, retrieving snippets then calling `chat_text`/`chat_structured`, with `temperature`, `top_k`, and `system_prompt_variant` config knobs
...
- [x] 7.1 Define `EvalCase` model (`id`, `input`, `expected`) in `refinely/datasets/loader.py`
...
- [x] 8.1 Implement `refinely/eval/metrics.py`: `Metric` base/protocol with `evaluate(case, output) -> MetricResult`
...
- [x] 9.1 Implement `refinely/eval/runner.py`: `EvaluationRunner.run(dataset, app, config)` executing every case through `app.execute`, collecting per-case output/expected/score, tolerating individual case failures without aborting the run
...
- [x] 10.1 Implement `refinely/tracking/db.py`: SQLite schema initialization for `evaluation_runs`, `metric_results`, `case_results` (idempotent — safe to call against an existing DB file, including one already containing Optuna's internal tables)
...
- [x] 11.1 Implement `refinely/optimize/search_space.py`: per-app Optuna search-space sampling functions (extraction: `temperature`, `system_prompt_variant`; QA: `temperature`, `top_k`, `system_prompt_variant`)
- [x] 11.2 Implement `refinely/optimize/objective.py`: `build_objective(app, dataset, lineage_db_path)` returning an Optuna objective function that samples a config, runs `EvaluationRunner`, records the run to lineage with `optuna_trial_number` set, and returns `aggregate_score`
...
- [x] 12.1 Implement `refinely/cli.py` with `click` group `main` and two subcommands: `evaluate <app>` and `optimize <app> [--trials N]` (app is `extraction` or `qa`)
...
- [x] 12.4 Manual verification: run `refinely evaluate extraction`, `refinely evaluate qa`, `refinely optimize extraction --trials 3`, `refinely optimize qa --trials 3` against the real OpenAI API and confirm sensible output and lineage rows are written

## 13. Final Verification

- [x] 13.1 Run the full unit test suite (`pytest`) and confirm all tests pass with zero live API calls
- [x] 13.2 Run `refinely optimize extraction --trials 15` and `refinely optimize qa --trials 15` end-to-end; confirm `aggregate_score` trends upward (or at least varies meaningfully) across trials via a lineage query
