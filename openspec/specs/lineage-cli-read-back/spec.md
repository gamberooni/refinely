...
- **WHEN** a user runs `crucible show <app> --tag <tag>`
...
The system SHALL support `crucible show <app> --run <run_id>` to render the run's per-case results as a table, ordered by case score ascending (worst cases first), with case id, score, input, expected, output, and any per-case error. The output SHALL report how many cases errored.
...
- **WHEN** a user runs `crucible show <app> --run <run_id>` and some of the run's cases have a recorded error
...
- **WHEN** a user runs `crucible compare <app> --tag <tag>`
...
- **WHEN** a user runs `crucible export <app> --tag <tag>`
...
- **WHEN** a user runs `crucible compare <app> --model <name>`
- **THEN** the table SHALL include only runs whose `model_name` matches `<name>`

#### Scenario: Filtering with no matching runs
- **WHEN** a user runs `crucible compare <app> --model <name>` and no recorded run uses that model
...
- **WHEN** a user runs `crucible compare <app> --diff-config` and the compared runs' configurations differ
...
- **WHEN** a user runs `crucible compare <app> --diff-config` and a compared run's configuration equals the baseline's
...
- **WHEN** a user runs `crucible compare <app> --cases` and both the baseline and the most recent run have recorded case results
...
- **WHEN** a user runs `crucible compare <app> --cases` and the two runs' `dataset_version` values differ
...
- **WHEN** a user runs `crucible compare <app> --cases` and one of the two runs has no recorded case results
