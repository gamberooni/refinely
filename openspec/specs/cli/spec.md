...
The CLI SHALL support a `show` subcommand (`crucible show <app>`) that reads the lineage database and renders the app's run history, and (`crucible show <app> --run <run_id>`) per-case results for a specific run.
...
- **WHEN** a user runs `crucible show <app> --run <run_id>`
- **THEN** the CLI SHALL render the run's per-case results, worst cases first

### Requirement: Compare runs via CLI
The CLI SHALL support a `compare` subcommand (`crucible compare <app> [--baseline <run_id>]`) that renders the app's runs with per-metric deltas against a baseline run.
