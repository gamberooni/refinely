...
## Purpose
TBD - created by archiving change actionable-results. Update Purpose after archive.
## Requirements
...
The system SHALL accept a `--tag` filter on the run read-back commands (`show`, `compare`, `export`) and SHALL restrict the returned runs to those bearing that tag. When no run bears the tag, the command SHALL report that no matching runs were found.

#### Scenario: Showing only tagged runs
- **WHEN** a user runs `crucible show <app> --tag candidate`
- **THEN** the CLI SHALL render only the runs tagged `candidate`

#### Scenario: Exporting only tagged runs
- **WHEN** a user runs `crucible export <app> --tag prod`
- **THEN** the CLI SHALL write the export containing only the runs tagged `prod`

#### Scenario: Tag filter matches nothing
- **WHEN** a user runs `crucible show <app> --tag <tag>` and no run of that app bears the tag
