...
- **WHEN** a user runs `crucible doctor --network`
...
- **WHEN** a user runs `crucible dataset stats <app>` and some cases are structurally inconsistent (e.g. a case missing keys present in the majority of cases, or an `expected` of a different type than the norm)
- **THEN** the CLI SHALL list the affected case ids in a malformed-case report

#### Scenario: Dataset that fails to parse
- **WHEN** a user runs `crucible dataset stats <app>` and the dataset file is missing, not valid JSON, or contains cases failing `load_dataset` validation
