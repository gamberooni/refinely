...
from crucible.devtools.doctor import CheckResult, run_checks
from crucible.devtools.scaffold import ScaffoldError, write_app
from crucible.eval.datasets import dataset_stats
...
def _hermetic_settings() -> None:
    ...
    crucible.core.settings.Settings.model_config["env_file"] = None
    yield


class TestScaffold:
    def test_write_app_creates_module_and_stub(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setattr("crucible.devtools.scaffold.APPS_DIR", tmp_path / "apps")
        monkeypatch.setattr("crucible.devtools.scaffold.DATASETS_DIR", tmp_path / "datasets")
...
class TestDoctor:
    def test_run_checks_default_has_no_network(self, monkeypatch) -> None:
        monkeypatch.setattr("crucible.devtools.doctor._check_apps", lambda: CheckResult("apps", True, "ok"))
        monkeypatch.setattr(
            "crucible.devtools.doctor._check_datasets", lambda: CheckResult("datasets", True, "ok")
        )
        monkeypatch.setattr(
            "crucible.devtools.doctor._check_schema", lambda s: CheckResult("schema", True, "ok")
        )
        monkeypatch.setattr("crucible.devtools.doctor._check_env", lambda s: CheckResult("env", True, "ok"))
    ...
    def test_run_checks_network_only_when_flagged(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("crucible.devtools.doctor._check_env", lambda s: CheckResult("env", True, "ok"))
        monkeypatch.setattr(
            "crucible.devtools.doctor._check_network",
...
class TestDeveloperCli:
    ...
    def test_new_app_command(self, tmp_path: Path, monkeypatch) -> None:
        ...
        assert 'crucible.apps' in result.output
    ...
    def test_doctor_all_pass_exits_zero(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "crucible.cli.devtools.run_checks",
    ...
    def test_doctor_failure_exits_one_with_hint(self, monkeypatch) -> None:
        monkeypatch.setattr(
            "crucible.cli.devtools.run_checks",
            lambda settings, network=False: [
                CheckResult("env", False, "CRUCIBLE_OPENAI_API_KEY is not set", hint="set it"),
            ],
        )
